...
export class NoteComponent {
  ...
  onPageChange(event: PageEvent): void {
    ...
    this.documentService.getAllNotes(page, pageSize).subscribe((data: any[]) => {
      ...
      this.notes.forEach(note => {
        note.safeUrl = this.sanitizer.bypassSecurityTrustResourceUrl(`http://127.0.0.1:8000${note.file}`);
      });
    });
  }
  ngOnInit(): void {
  ...
  deleteDocument(noteId: number) {
    const url = `${API_BASE_URL}delete-note/${noteId}`;
